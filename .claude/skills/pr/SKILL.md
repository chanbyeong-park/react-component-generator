...
- 커밋 여러 개 → 변경사항 전체를 종합해 50자 이내로 작성

**본문 작성:**
`references/pr-template.md`를 읽어 각 섹션을 변경사항 기반으로 채운다.
- "변경 사항 요약": 이 PR이 왜 필요한지 1~2문장
- "주요 변경 내역": 커밋 로그와 diff --stat 기반으로 bullet 작성
- "테스트 방법": 변경된 기능을 확인하는 구체적인 방법
- "관련 이슈": 커밋 메시지에서 `#번호` 패턴을 찾아 자동 추출, 없으면 "해당 없음"
